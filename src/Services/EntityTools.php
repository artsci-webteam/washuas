...
namespace Drupal\washuas\Services;
 ...
 * Class EntityTools.
 *
 * @file providing helpful Entity functions to share across washuas module and submodules
 */
...
class EntityTools {
  ...
  public function addTaxonomyFieldToBundle($entity_type, $bundle, $field_name, $label, $required, $description, $vocabulary, $widget_type, $default_value = NULL): void {
    ...
    if ($field_exists) {
      ...
      if (empty($field)) {
        ...
        $this->loggerFactory->get('washuas')->notice($message);
  ...
  public function addFieldtoEntityForm($entity_type, $bundle, $field_name, $widget_type): void {
    ...
    if ($displayForm->save()) {
      ...
      $this->loggerFactory->get('washuas')->error($message);
  ...
  public function updateNodesByIDs($nodeID,$fields):int{
    ...
    foreach ( $nodes as $node ){
      ...
      foreach ( $fields as $field => $value ){
        ...
        switch( $type ){
          case "entity_reference_revisions": //for fields that consist of paragraphs we'll delete the old paragraphs before updating
            //get all of the paragraph ids from the field, then delete 'em
          ...
          case "string":
          default:
            if (!empty($value)) $node->set($field, $value);//update the data with the new value if it's not empty
            break;
        }
      }
